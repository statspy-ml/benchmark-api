...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Benchmark FastAPI vs Litestar vs Go  ${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
collect_docker_stats() {
    ...
    while true; do
        ...
        # Parse do output do docker stats
        ...
        # Converte limit de GiB para MiB
...
run_test() {
    ...
    # Arquivo de métricas do Docker
    local docker_stats_file="${RESULTS_DIR}/${api}_docker_stats_${TIMESTAMP}.csv"
    ...
    # Aguarda 3 segundos para garantir que a coleta começou
    sleep 3

    # Roda o K6
    echo -e "${BLUE}→ Running K6 load test...${NC}"

    # Define UID/GID para o container k6 ter permissões corretas
    export USER_ID=$(id -u)
    export GROUP_ID=$(id -g)

    # Executa k6 com output JSON e summary
    docker compose run --rm k6 run /scripts/load-test.js \
        -e API="$api" \
        -e TIMESTAMP="${TIMESTAMP}" \
        --out json="/results/${api}_metrics_${TIMESTAMP}.json" \
        --summary-export="/results/${api}_summary_${TIMESTAMP}.json" \
        2>&1 | sed -r "s/\x1B\[[0-9;]*[mK]//g" | tee "${RESULTS_DIR}/${api}_output_${TIMESTAMP}.log"
    ...
    # Pausa entre testes para containers esfriarem
    if [ "$api" != "go-gin" ]; then
...
docker compose ps | grep -q "benchmark-fastapi.*Up" || {
    echo -e "${RED}✗ Containers not running. Starting them...${NC}"
    docker compose up -d fastapi litestar go go-fiber go-gin
...
ls -lh "$RESULTS_DIR"/*"${TIMESTAMP}"* 2>/dev/null | awk '{print "  - " $9}'
echo ""
echo -e "${BLUE}To analyze results, check the JSON and CSV files.${NC}"
echo ""

# Sugestão de próximos passos
echo -e "${YELLOW}Next steps:${NC}"
echo "  1. Analyze K6 JSON files for HTTP metrics (P50, P95, P99, RPS)"
echo "  2. Analyze CSV files for resource usage (CPU, Memory)"
echo "  3. Compare results across frameworks"
echo ""
